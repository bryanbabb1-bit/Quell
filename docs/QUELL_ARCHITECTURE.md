...
|---|---|---|
| 2026-06-15 | `a86043d` | **DIRECTION PIVOT → people-first community app (Strava energy).** Research-backed: the best community apps are people/moment-first. (1) New **"Ignite" palette** — light warm-white base + energetic orange accent — replaces black/gold as the default (Members dark still selectable); `cinematicColors` keeps reveal/live **always dark** for contrast. (2) Swept black/gold remnants (MatchDeck de-gilded, confetti, app.json — icon/splash art still needs a rebuild). (3) **Moment-card feed**: matches render as people-first cards (faces + result hero line + 🔥 **kudos**); `GET /matches/feed` now returns `cheer_count`/`viewer_cheered`. NEXT: comments; profiles as a destination; reveal share-out. |
| 2026-06-15 | (immersion) | **Immersion Phase 1 — motion foundation + Feed** (motion kit later set aside — the cascade wasn't a value-add; PressableScale/CountUp kept) (app-only/OTA): reusable motion kit (`constants/motion.ts` tokens; `components/motion/` — `CountUp` [extracted from reveal's useCountUp], `Reveal`/`Stagger` FadeInUp entrances, `PressableScale` generalizing the Button press-spring; all honor reduce-motion). Applied to the Feed: sections cascade in on load (`<Reveal index>`), pulse stats count up, the day's results crossfade on date change, and rows/pills/buttons/masthead depress with haptics. Reveal refactored onto the shared `useCountUp`. Spreads the reveal's fluidity vocabulary; rolls out to more surfaces next. (Plan: `~/.claude/plans/peppy-soaring-steele.md`) |
| 2026-06-15 | (handicaps) | **Plus-handicap display + handicapped demo seed**: a "plus" handicap (better than scratch, stored negative) was rendering as "-1" instead of "+1"; new `formatCourseHandicap` (whole-stroke +N convention) applied to the live header pill, match screen CH, member index, and intro reason. Live-demo seed now carries real handicaps (incl. a +1 and a +2) so the gamecast allocates strokes/pops to test with |
| 2026-06-15 | (live polish) | **Live-match polish (4 items)**: (1) reactions are now a **toggle** — tap again to remove (migration 0021: one per match/user/kind); **long-press** (participants) opens a "who reacted" sheet (`GET /matches/:id/reactors`, `your_reactions` on live state). (2) **Course handicaps** shown by each name in the live header (`computeLiveState` attaches each player's CH to the gamecast). (3) **Full scorecard** — all 18 holes, with a gold **pop dot** marking where each player gets a handicap stroke, so you see pops coming. (4) **Pinned note** now actually displays — surfaced on the club board masthead (was set in Club Control but read nowhere) |
| 2026-06-15 | (intros) | **Suggested intros — the club as matchmaker** (`lib/intros.ts`, `GET /clubs/:id/intros`, staff-gated): surfaces members active in the last 90 days who've **never played each other**, scored by handicap closeness + mutual engagement + both-recent + a newcomer↔regular "welcome" bridge, diversified (≤2 appearances each), top 6. New panel in Club Control with a per-row share (`shareIntro`) the pro can forward. Directly serves the "meet members" thesis |
| 2026-06-15 | (member detail) | **Member engagement detail + Club Control home cleanup**: Club Control entry removed from the feed masthead (lives only in the hamburger now); new staff-only **member engagement screen** (`GET /clubs/:id/member/:uid`, `lib/memberDetail.ts`) — deliberately NO win/loss; shows status (new/active/cooling/lapsed) + plain-English headline, activity (totals, per-week, 8-week trend, rising/cooling momentum), and the social signal that's the real product: **"N members met here" + top playing partners**; tap any name in the dashboard's most-active or lapsing lists to open it |
| 2026-06-15 | `5c54f52` | **Club Control + invite loop** (app-only OTA): hamburger gains a staff-gated **Club Control** entry (driven by `staff_club_id`) and an everyone **Invite a friend** share; "Manage Club" renamed **Club Control** throughout; dashboard gains a gold **"Grow your board"** recruit CTA (club-branded share) and relabels its course-activity metric "live"→"on the course"; new `lib/invite.ts` (`FORETERA_URL`, `sharePlayerInvite`, `shareClubInvite`); sign-in/hamburger tagline "Post a match"→**"Find a match"** |
| 2026-06-15 | `a649c2f` | **Net-aware scoring + watchable "live now"**: course-pulse `live_now` now counts only watchable matches (public + same-group + today), feed live filter mirrors it; play-by-play says "net birdie" when a stroke fell + scorecard shows gross(net); `GamecastHole` gains creator/opponent net + strokes (both push sites; 30 engine tests) |
| 2026-06-14 | (prior) | **Pre-beta hardening** (`SCALABILITY_REVIEW.md`): verified remote D1 has all 0014–0019 indexes; added `idx_matches_course_status` (migration 0020 — the leaderboard/champions/dashboard/pulse index); `client_logs` 30-day retention in the hourly cron. Bottom line: stack is fine for 50–500 beta testers; gamecast polling cache is the pre-live-event item |
