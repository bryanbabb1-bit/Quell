...
|---|---|---|
| 2026-06-15 | (this push) | **Live-match polish (4 items)**: (1) reactions are now a **toggle** — tap again to remove (migration 0021: one per match/user/kind); **long-press** (participants) opens a "who reacted" sheet (`GET /matches/:id/reactors`, `your_reactions` on live state). (2) **Course handicaps** shown by each name in the live header (`computeLiveState` attaches each player's CH to the gamecast). (3) **Full scorecard** — all 18 holes, with a gold **pop dot** marking where each player gets a handicap stroke, so you see pops coming. (4) **Pinned note** now actually displays — surfaced on the club board masthead (was set in Club Control but read nowhere) |
