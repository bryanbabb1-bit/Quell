...
|---|---|---|
| 2026-06-15 | (this push) | **Plus-handicap display + handicapped demo seed**: a "plus" handicap (better than scratch, stored negative) was rendering as "-1" instead of "+1"; new `formatCourseHandicap` (whole-stroke +N convention) applied to the live header pill, match screen CH, member index, and intro reason. Live-demo seed now carries real handicaps (incl. a +1 and a +2) so the gamecast allocates strokes/pops to test with |
