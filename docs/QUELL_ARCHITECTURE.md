...
|---|---|---|
| 2026-06-15 | (this push) | **Suggested intros — the club as matchmaker** (`lib/intros.ts`, `GET /clubs/:id/intros`, staff-gated): surfaces members active in the last 90 days who've **never played each other**, scored by handicap closeness + mutual engagement + both-recent + a newcomer↔regular "welcome" bridge, diversified (≤2 appearances each), top 6. New panel in Club Control with a per-row share (`shareIntro`) the pro can forward. Directly serves the "meet members" thesis |
